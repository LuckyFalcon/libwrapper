...
const workerFarm = require('worker-farm')
    , workers    = workerFarm({maxConcurrentWorkers : 3, maxRetries: 1000}, require.resolve(path.join(process.cwd(), "/services/getAttractor/forkedlongComputation.js")))
 ...
 (req, res, next) => { /* the rest of the existing function */ 
  ...
  if (!errors.isEmpty()) {
    ...
    if (req.query.gid && !req.query.size){ 
      ...
      anuQrng.getentropy(req.query.gid, req.query.pool, req.query.temporal, req.query.gcp, function(result) {
        if (result == "1") { //File not found
            res.end(JSON.stringify("GID was not found"));     
        } else {//File is found, check for raw.
        //Set the content type and status code
